...
def pagina_chat():
    st.header('Bem-vindo a IA Estratégica 🚀🚀🚀', divider=True)

    chain = st.session_state.get('chain')
    if chain is None:
        st.error('Carrege a IA')
        st.stop()
    ...
    input_usuario = st.chat_input('Converse com a IA')
    if input_usuario:
...
def sidebar():
    ...
    if st.button('Inicializar a IA Estratégica', use_container_width=True):
